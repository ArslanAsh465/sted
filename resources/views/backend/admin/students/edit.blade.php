        ...
        <form action="{{ route('admin.students.update', $student->id) }}" method="POST">
                ...
                <select name="parent_id" id="parent_id" class="form-select @error('parent_id') is-invalid @enderror" required>
                    ...
                    @foreach($parents as $parent)
                        <option value="{{ $parent->id }}"
                            {{ old('parent_id', $student->student->parent_id ?? '') == $parent->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="first_name" id="first_name"
                       value="{{ old('first_name', $student->student->first_name ?? '') }}"
            ...
            <div class="mb-3">
                ...
                <input type="text" name="last_name" id="last_name"
                       value="{{ old('last_name', $student->student->last_name ?? '') }}"
            ...
            <div class="mb-3">
                ...
                <input type="text" name="roll_no" id="roll_no"
                       value="{{ old('roll_no', $student->student->roll_no ?? '') }}"
            ...
            <div class="mb-3">
                ...
                <input type="date" name="date_of_birth" id="date_of_birth"
                       value="{{ old('date_of_birth', $student->student->date_of_birth ?? '') }}"
            ...
            <div class="mb-3">
                ...
                <textarea name="address" id="address" rows="3"
                          class="form-control @error('address') is-invalid @enderror" required>{{ old('address', $student->student->address ?? '') }}</textarea>
                @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <select name="country_id" id="country_id" class="form-select @error('country_id') is-invalid @enderror" required>
                    ...
                    @foreach($countries as $country)
                        <option value="{{ $country->id }}"
                            {{ old('country_id', $student->student->country_id ?? '') == $country->id ? 'selected' : '' }}>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="state_id" id="state_id" class="form-select @error('state_id') is-invalid @enderror" required>
                    ...
                    @foreach($states as $state)
                        @if(old('country_id', $student->student->country_id ?? '') == $state->country_id)
                            <option value="{{ $state->id }}"
                                {{ old('state_id', $student->student->state_id ?? '') == $state->id ? 'selected' : '' }}>
                                {{ $state->name }}
                            </option>
                        @endif
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <select name="city_id" id="city_id" class="form-select @error('city_id') is-invalid @enderror" required>
                    ...
                    @foreach($cities as $city)
                        @if(old('state_id', $student->student->state_id ?? '') == $city->state_id)
                            <option value="{{ $city->id }}"
                                {{ old('city_id', $student->student->city_id ?? '') == $city->id ? 'selected' : '' }}>
                                {{ $city->name }}
                            </option>
                        @endif
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <input type="text" name="postal_code" id="postal_code"
                       value="{{ old('postal_code', $student->student->postal_code ?? '') }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function populateStates(countryId) {
            ...
            const oldState = "{{ old('state_id', $student->student->state_id ?? '') }}";
            if (oldState && countryId == "{{ old('country_id', $student->student->country_id ?? '') }}") {
        ...
        function populateCities(stateId) {
            ...
            const oldCity = "{{ old('city_id', $student->student->city_id ?? '') }}";
            if (oldCity && stateId == "{{ old('state_id', $student->student->state_id ?? '') }}") {
        ...
        const initialCountry = "{{ old('country_id', $student->student->country_id ?? '') }}";
        if (initialCountry) {
