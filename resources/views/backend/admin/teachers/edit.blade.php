...
@section('content')
    ...
    <div class="card mt-4">
        <div class="card-header">
            <h5 class="mb-0">Update Teacher Details</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.teachers.update', $teacher->id) }}" method="POST">
                    ...
                    <select name="institute_id" id="institute_id" class="form-select @error('institute_id') is-invalid @enderror" required>
                        ...
                        @foreach($institutes as $institute)
                            <option value="{{ $institute->institute->id ?? '' }}" 
                                {{ old('institute_id', $teacher->teacher->institute_id ?? '') == ($institute->institute->id ?? '') ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="first_name" id="first_name" 
                        value="{{ old('first_name', $teacher->teacher->first_name ?? '') }}" 
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="last_name" id="last_name" 
                        value="{{ old('last_name', $teacher->teacher->last_name ?? '') }}" 
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="specialization" id="specialization" 
                        value="{{ old('specialization', $teacher->teacher->specialization ?? '') }}" 
                ...
                <div class="mb-3">
                    ...
                    <textarea name="address" id="address" rows="3" 
                            class="form-control @error('address') is-invalid @enderror" required>{{ old('address', $teacher->teacher->address ?? '') }}</textarea>
                    @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <select name="country_id" id="country_id" class="form-select @error('country_id') is-invalid @enderror" required>
                        ...
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" 
                                {{ old('country_id', $teacher->teacher->country_id ?? '') == $country->id ? 'selected' : '' }}>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="state_id" id="state_id" class="form-select @error('state_id') is-invalid @enderror" required>
                        ...
                        @foreach($states as $state)
                            @if(old('country_id', $teacher->teacher->country_id ?? '') == $state->country_id)
                                <option value="{{ $state->id }}" 
                                    {{ old('state_id', $teacher->teacher->state_id ?? '') == $state->id ? 'selected' : '' }}>
                                    {{ $state->name }}
                                </option>
                            @endif
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <select name="city_id" id="city_id" class="form-select @error('city_id') is-invalid @enderror" required>
                        ...
                        @foreach($cities as $city)
                            @if(old('state_id', $teacher->teacher->state_id ?? '') == $city->state_id)
                                <option value="{{ $city->id }}" 
                                    {{ old('city_id', $teacher->teacher->city_id ?? '') == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}
                                </option>
                            @endif
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="postal_code" id="postal_code" 
                        value="{{ old('postal_code', $teacher->teacher->postal_code ?? '') }}" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function populateStates(countryId) {
            ...
            // Restore old or current state selection
            const oldState = "{{ old('state_id', $teacher->teacher->state_id ?? '') }}";
            if (oldState && countryId == "{{ old('country_id', $teacher->teacher->country_id ?? '') }}") {
        ...
        function populateCities(stateId) {
            ...
            // Restore old or current city selection
            const oldCity = "{{ old('city_id', $teacher->teacher->city_id ?? '') }}";
            if (oldCity && stateId == "{{ old('state_id', $teacher->teacher->state_id ?? '') }}") {
        ...
        // Initial population on page load (for old or current values)
        const initialCountry = "{{ old('country_id', $teacher->teacher->country_id ?? '') }}";
        if (initialCountry) {
